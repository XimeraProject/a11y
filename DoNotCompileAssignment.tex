...
\begin{selectAll}%% This should render automatically as checkboxes
	...
	\choice{$x=1$}
	\choice{$x=-1$}
	\choice{$x=\sqrt{2}\alt{x equals the square root of 2}$}
...
\end{align*}
  ...
  Which of the following best describes the end behavior of $f$ as $x \to \infty\alt{x goes to infinity}$?		
  \begin{multipleChoice} %% This should render automatically as radiobuttons
    ...
    \choice{$f$ has some other behavior at $\infty\alt{infinity}$.}
  \end{multipleChoice}
...
function is \wordChoice{\choice[correct]{increasing} %% \wordChoice needs to default to radio buttons
...
\begin{image}\alt{We add arrows showing where the function is increasing and decreasing to the plot from before.  The arrows point up until $x=-1$, then down until $x=2$, and finally up from that point on.}
\begin{tikzpicture}
	\begin{axis}[
            axis on top=true,
            domain=-2:4,
...
\begin{image}\alt{We add a curve to the plot from before. The curve is concave down until the origin. It is then concave up from then on. There is a local maximum at x=-1 and a local minimum at x=2.}
\begin{tikzpicture}
	\begin{axis}[
            axis on top=true,
            domain=-2:4,
